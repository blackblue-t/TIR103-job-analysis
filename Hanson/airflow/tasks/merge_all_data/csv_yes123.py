...
def csv_123():
    # 讀取CSV文件
    df = pd.read_csv('opt/airflow/tasks/raw_data4merge/all_raw_data_123.csv',encoding= 'utf8')
    df = df.drop(columns=['required_major', 'job_skills','category_primary','category_secondary','job_url'])
    # 指定新的欄位順序
    ...
    def transform_experience(text):
        if isinstance(text, str):  # 確認 text 是否為字串
            if re.search(r"不拘", text):
                text = 0
            elif re.search(r"(\d+)年以上", text): 
                text = re.sub(r"(\d+)年以上", r'\1', text)
    ...
    df.to_csv('opt/airflow/tasks/trans_data/data_123.csv', index=False, encoding='utf-8')
