...
def csv_104():
  # 讀取CSV文件
  df = pd.read_csv('opt/airflow/tasks/raw_data4merge/all_raw_data_104.csv',encoding= 'utf8')
  ...
  def transform_experience(text):
      if isinstance(text, str):
          ...
          if re.search(r"(\d+)\s*年以上", text):
              # 先處理 "X年工作經驗以上" 替換為 "X年以上"
              text = re.sub(r"(\d+)\s*年以上", r"\1", text)
  ...
  # 將結果存回 CSV 檔案
  output_path = 'opt/airflow/tasks/trans_data/data_104.csv'  # 替換為輸出的檔案路徑
